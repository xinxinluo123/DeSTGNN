...
class spatialAttentionScaledGCN(nn.Module):
    def __init__(self, sym_norm_Adj_matrix, in_channels, out_channels,train_random,eval_random,dropout=.0):
    ...
    def forward(self, x,eval_random):
        ...
        causal_output = random_output(context_output,noncausal_output, eval_random)
...
class SublayerConnection(nn.Module):
    ...
    def forward(self, x, sublayer, eval_random):
        '''
        ...
        if self.residual_connection and self.use_LayerNorm:
            return x + self.dropout(sublayer(self.norm(x), eval_random))
...
class PositionWiseGCNFeedForward(nn.Module):
    ...
    def forward(self, x, eval_random):
        ...
        :return: (B, N, T, F_out)
        '''
        return self.dropout(F.relu(self.gcn(x, eval_random)))
...
class MultiHeadAttentionAwareTemporalContex_qc_kc(nn.Module):  # key causal; query causal;
    ...
    def forward(self, query, key, value, mask=None, query_multi_segment=False, key_multi_segment=False,eval_random=False):
        ...
        causal_output = random_output(context_x,noncausal_x, eval_random)
...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, src, trg,train_random):
        '''
        src:  (batch_size, N, T_in, F_in)
        trg: (batch, N, T_out, F_out)
        '''
        encoder_output = self.encode(src,train_random)  # (batch_size, N, T_in, d_model)

        return self.decode(trg, encoder_output,train_random)

    def encode(self, src,eval_random):
        '''
        src: (batch_size, N, T_in, F_in)
        '''
        h = self.src_embed(src)
        return self.encoder(h, eval_random)
        # return self.encoder(self.src_embed(src))

    def decode(self, trg, encoder_output, eval_random):
        '''
        counterfactual
        ...
        decoder=self.decoder(self.trg_embed(trg), encoder_output,eval_random)
        
        decoder_c=self.decoder(self.trg_embed(trg), encoder_output_c,eval_random)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, eval_random):
        ...
        if self.residual_connection or self.use_LayerNorm:
            x = self.sublayer[0](x,  lambda x, eval_random: self.self_attn(x, x, x, query_multi_segment=True, key_multi_segment=True, eval_random=eval_random), eval_random)
            return self.sublayer[1](x, self.feed_forward_gcn, eval_random)
        else:
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, memory, eval_random):
        '''
        :param x: (batch_size, N, T', F_in)
        ...
        if self.residual_connection or self.use_LayerNorm:
            x = self.sublayer[0](x, lambda x,eval_random: self.self_attn(x, x, x, tgt_mask, query_multi_segment=False, key_multi_segment=False,eval_random=eval_random), eval_random)  # output: (batch, N, T', d_model)
            x = self.sublayer[1](x, lambda x,eval_random: self.src_attn(x, m, m, query_multi_segment=False, key_multi_segment=True,eval_random=eval_random), eval_random)  # output: (batch, N, T', d_model)
            return self.sublayer[2](x, self.feed_forward_gcn, eval_random)  # output:  (batch, N, T', d_model)
        else:
...
def make_model(DEVICE, num_layers, encoder_input_size, decoder_output_size, d_model, adj_mx, nb_head, num_of_weeks,
               ...
               ScaledSAt=True, SE=True, TE=True, kernel_size=3, smooth_layer_num=0, residual_connection=True, use_LayerNorm=True,train_random=True,eval_random=False):
    ...
    if ScaledSAt:  # employ spatial self attention
        position_wise_gcn = PositionWiseGCNFeedForward(spatialAttentionScaledGCN(norm_Adj_matrix, d_model, d_model,train_random,eval_random), dropout=dropout)
    else:  # 不带attention
