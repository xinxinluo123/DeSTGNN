...
import argparse
import configparser
from model.DeSTGNN import make_model
...
parser.add_argument("--config", default='./configurations/PEMS08.conf', type=str, help="configuration file path")
...
train_random = bool(int(training_config['train_random']))
eval_random = bool(int(training_config['eval_random']))
use_LayerNorm = True
...
if train_random:
    folder_dir = folder_dir + 'train_random'
if eval_random:
    folder_dir = folder_dir + 'eval_random'    
...
net = make_model(DEVICE, num_layers, encoder_input_size, decoder_input_size, d_model, adj_mx, nb_head, num_of_weeks,
                 num_of_days, num_of_hours, points_per_hour, num_for_predict, dropout=dropout, aware_temporal_context=aware_temporal_context, ScaledSAt=ScaledSAt, SE=SE, TE=TE, kernel_size=kernel_size, smooth_layer_num=smooth_layer_num, residual_connection=residual_connection, use_LayerNorm=use_LayerNorm,train_random= train_random,eval_random=eval_random)
...
def train_main():
    ...
    for epoch in range(start_epoch, epochs):
        ...
        val_loss = compute_val_loss(net, val_loader, criterion, sw, epoch,eval_random)
        ...
        for batch_index, batch_data in enumerate(train_loader):
            ...
            outputs = net(encoder_inputs, decoder_inputs,train_random)
    ...
    predict_main(best_epoch, test_loader, test_target_tensor, _max, _min, 'test',eval_random)
    ...
    for epoch in range(epochs, epochs+fine_tune_epochs):
        ...
        for batch_index, batch_data in enumerate(train_loader):
            ...
            encoder_output = net.encode(encoder_inputs,train_random)
            ...
            for step in range(predict_length):
                ...
                predict_output = net.decode(decoder_inputs, encoder_output,train_random)
...
def predict_main(epoch, data_loader, data_target_tensor, _max, _min, type,eval_random):
    '''
    在测试集上，测试指定epoch的效果
    :param epoch: int
    ...
    predict_and_save_results(net, data_loader, data_target_tensor, epoch, _max, _min, params_path, type,eval_random)
